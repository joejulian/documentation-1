...
1. Migrate your existing Redpanda Helm chart configuration to the Redpanda custom resource.
  ...
  Configure all your custom overrides in the `clusterSpec` field. The Redpanda Operator passes these values to the Redpanda Helm chart. The keys in the `clusterSpec` field are the same as those in the Redpanda Helm chart.
